...
않은 별도 적응형 세트(adaptive_payloads.py, **10종**)로 측정한다. 게이트 강화
(NFKC+Tags-block/bidi 정규화, base64/ROT13/hex **decode-재검사**, 마크다운-URL 무력화)
후 탐지율은 **60%(6/10)** — 제로폭/Tags-block/인코딩/마크다운exfil 변형은 잡고,
**남는 40%는 순수 의미 패러프레이즈**(평문/철자분해/영문)로 키워드 게이트의 구조적
한계다. 이건 `review(use_llm=True)` 선택적 2차 LLM 심판(로컬 ollama)으로만 보강된다.
(자가-저작 5종의 4/4=100% 와 달리, 이 60% 가 '정직한 일반화 수치'다.)
