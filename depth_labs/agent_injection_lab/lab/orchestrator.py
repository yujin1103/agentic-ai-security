...
class ChainOrchestrator:
    def __init__(self, roles, llm: LLMClient, tools: ToolRegistry, secret: str,
                 ...
                 egress_guard: bool = False, llms: list[LLMClient] | None = None):
        self.roles = roles
        self.llm = llm
        self.tools = tools
        self.secret = secret
        ...
        # llms: 에이전트별 서로 다른 모델(이종 체인=모델 간 전파). None 이면 전원 self.llm.
        self.llms = llms
    ...
    def run_once(self, trial_seed: int, poison_index: int = 0) -> TrialResult:
        ...
        for i, (role, instr) in enumerate(self.roles):
            ...
            llm_i = self.llms[i % len(self.llms)] if self.llms else self.llm
            agent = Agent(i, role, instr, llm_i, self.tools, self.secret,
